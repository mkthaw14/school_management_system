    ...
    <div class="row justify-content-center">
        <div class="col-lg-7">
            <div class="card shadow-lg border-0 rounded-lg mt-5">
                <div class="card-header"><h3 class="text-center font-weight-light my-4">Edit Subject Assignment</h3></div>
                <div class="card-body">
                                ...
                                <div class="form-floating mb-3 ">
                                    <select class="form-control @error('subject_id') is-invalid @enderror" name="subject_id">
                                        @foreach ($subjects as $v)
                                            <option value="{{$v->id}}" @if($v->id == $edit_subject->id) {{"selected"}} @endif >{{$v->name}}</option> 
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-12">
                                <div class="form-floating mb-3 ">
                                    <select class="form-control @error('grade_id') is-invalid @enderror" name="grade_id">
                                        @foreach ($grades as $v)
                                            <option value="{{$v->id}}" @foreach ($edit_subject->subject_assignments as $sa)
                                                @if ($sa->grade_id == $v->id)
                                                    {{"selected"}}
                                                @endif
                                            @endforeach  >{{$v->name}} - {{$v->academic_year->yearOne}}</option> 
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3 ">
                                    ...
                                    <select class="form-control @error('teachers') is-invalid @enderror" name="teachers[]" multiple>
                                        @foreach ($teachers as $v)
                                            <option value="{{$v->id}}" @if($v->id == 1) {{"Yes"}} @endif > {{$v->name}} </option> 
                                        @endforeach
                                    </select>
